...
cd ../mission/heron
  ...
  ./launch_heron.sh blue drone 24 --behavior=DEFEND_FORWARD --color=gray $LOGGING $TIME_WARP > /dev/null &
  ./launch_heron.sh blue drone 25 --behavior=DEFEND_FORWARD --color=yellow $LOGGING $TIME_WARP > /dev/null &
cd ..
