...
def make_state(state_request, num_states, MOOS_STATE, enemy):
  state = [0.0]*num_states

  for key in state_request:
    entry = state_request[key]
    # Replace any evan in vehicles as the specified enemy
    # Translation from pLearn enviroment to AgentAquaticus
    if entry.vehicle == 'evan':
      entry.vehicle = enemy
