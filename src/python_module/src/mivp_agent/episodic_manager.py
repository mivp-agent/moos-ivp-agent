...
class EpisodicManager:
    ...
    Examples:
      It is recommended to use EpisodicManager with the python context manager

      ```
      from mivp_agent.manager import EpisodicManager

      with EpisodicManager('trainer') as mgr:
    ...
    def __init__(self, task, log=True, immediate_transition=True, log_whitelist=None, id_suffix=None, output_dir=None):
        '''
        The initializer for EpisodicManager

        Args:
            task (str): For organization of saved data type is required to specify what task the EpisodicManager is preforming. For example a `EpisodicManager('trainer')` will log data under `generated_files/trainer/` in the current working directory.
            ...
            while not self._stop_signal:
                ...
                for addr in server._clients:
                    ...
                    if msg is not None:
                        ...
                        assert address_map[msg[KEY_ID]] == addr, "Vehicle changed vname. This violates routing / logging assumptions made by EpisodicManager"
    ...
    def are_present(self, vnames):
        '''
        Used to see if a specified list of vehicles has connected to the `EpisodicManager` instance yet.

        See also: [`wait_for()`][mivp_agent.manager.EpisodicManager.wait_for]
    ...
    def wait_for(self, vnames, sleep=0.1):
        '''
        Used to block until a specified list of vehicles has connect to the `EpisodicManager` instance.
    ...
    def get_vehicle_count(self):
        '''
        Returns:
          int: The amount of vehicles that have connected to this instance of `EpisodicManager`
        '''
    ...
    def run(self, q, attack_actions, retreat_actions):
        ...
        while True:
          ...
          model_state = q.get_state(
            msg.state['NAV_X'],
            msg.state['NAV_Y'],
            msg.state['NODE_REPORTS']['drone_21']['NAV_X'],
            msg.state['NODE_REPORTS']['drone_21']['NAV_Y'],
            msg.state['HAS_FLAG']
          )
